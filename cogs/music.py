...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        clr = 14471423
        embed = discord.Embed(
            title="", description=f"Queued [{data['title']}]({data['webpage_url']}) [{ctx.author.mention}]", color=discord.Color(clr))
...
class MusicPlayer:
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            clr = 14471423
            embed = discord.Embed(
                title="Now playing", description=f"[{source.title}]({source.web_url}) [{source.requester.mention}]", color=discord.Color(clr))
...
class Music(commands.Cog):
    ...
    async def connect_(self, ctx, *, channel: discord.VoiceChannel = None):
        ...
        if not channel:
            try:
                ...
                clr = 14471423
                embed = discord.Embed(title="", description="No channel to join. Please call `" + pr2 + "join` from a voice / stage channel.", color=discord.Color(clr))
                await ctx.send(embed=embed)
        ...
        if vc:
            ...
            try:
                ...
                if channel.type == discord.ChannelType.stage_voice:
                    await channel.connect(timeout=5.0, reconnect=True)
        ...
        await ctx.message.reply(f'**Joined `{channel}`**', mention_author = False)
    ...
    async def move_(self, ctx, *, channel: discord.VoiceChannel = None):
        ...
        if not channel:
            await ctx.message.reply("You are not connected to any voice/stage channel.", mention_author = False)
    ...
    async def pause_(self, ctx):
        ...
        if not vc or not vc.is_playing():
            clr = 14471423
            embed = discord.Embed(
                title="", description="I am currently not playing anything", color=discord.Color(clr))
            return await ctx.send(embed=embed)
        elif vc.is_paused():
            return

        vc.pause()
        await ctx.message.reply("Paused ⏸️", mention_author = False)
    ...
    async def resume_(self, ctx):
        ...
        vc.resume()
        await ctx.message.reply("Resuming ⏯️", mention_author = False)
    ...
    async def remove_(self, ctx, pos: int=None):
        ...
        if pos == None:
            ...
            try:
                ...
                embed = discord.Embed(
                    title="", description=f"Removed [{s['title']}]({s['webpage_url']}) [{s['requester'].mention}]", color=discord.Color(clr))
                await ctx.send(embed=embed)
            except:
                embed = discord.Embed(
                    title="", description=f'Could not find a track for "{pos}"', color=discord.Color(clr))
                await ctx.send(embed=embed)
    ...
    async def queue_info(self, ctx):
        ...
        if player.queue.empty():
            embed = discord.Embed(
                title="", description="Queue is empty", color=discord.Color(clr))
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(
            title=f'Queue for {ctx.guild.name}', description=fmt, color=discord.Color(clr))
    ...
    async def now_playing_(self, ctx):
        ...
        embed = discord.Embed(
            title="", description=f"[{vc.source.title}]({vc.source.web_url}) [{vc.source.requester.mention}] | `{duration}`", color=discord.Color(clr))
    ...
    async def change_volume(self, ctx, *, vol: float =None):
        ...
        if not vc or not vc.is_connected():
            embed = discord.Embed(
                title="", description="I am not currently connected to voice", color=discord.Color(clr))
            return await ctx.send(embed=embed)

        if not vol:
            embed = discord.Embed(
                title="", description=f"🔊 **{(vc.source.volume)*100}%**", color=discord.Color(clr))
            return await ctx.send(embed=embed)

        if not 0 < vol < 101:
            embed = discord.Embed(
                title="", description="Please enter a value between 1 and 100", color=discord.Color(clr))
            return await ctx.send(embed=embed)
    ...
    async def leave_(self, ctx):
        ...
        await ctx.message.add_reaction('👋')
        await ctx.message.reply('**Successfully disconnected**', mention_author = False)
